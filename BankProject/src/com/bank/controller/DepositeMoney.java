		...
		try {
			...
			while (res.next()) {
				...
				if (blob != null) {
					...
					out.println("</tr>");
					out.println(
							"<tr><td colspan='4'><button type=\"submit\" class=\"btn btn-primary se\" id='mySubmit' onclick='myFunction()'>DEPOSITE MONEY</button></td>");
					out.println("</tr>");
				}
			}
			out.println("</table>");
			out.println("</form>");
			RequestDispatcher dispatcher = req.getRequestDispatcher("deposite.jsp");
